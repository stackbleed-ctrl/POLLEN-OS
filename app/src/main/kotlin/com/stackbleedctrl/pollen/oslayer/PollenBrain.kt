    ...
    suspend fun decide(event: PhoneEvent): BrainDecision {
        ...
        val decision = when {
            raw.startsWith("OPEN_APP:") ->
                BrainDecision(DecisionType.OPEN_APP, "Open app", raw.removePrefix("OPEN_APP:"))

            raw.startsWith("MESH_FORWARD:") ->
                BrainDecision(DecisionType.MESH_FORWARD, "Forward to nearby", raw.removePrefix("MESH_FORWARD:"))

            raw == "BLOCK_CALL" ->
                BrainDecision(DecisionType.BLOCK_CALL, "Block suspicious call")

            raw == "SUMMARIZE" ->
                BrainDecision(DecisionType.SUMMARIZE, "Summarize current context")

            else ->
                BrainDecision(DecisionType.LOG_ONLY, raw)
        }

        memory.append(event.summary(), decision.summary)
        if (decision.type == DecisionType.MESH_FORWARD) {
            swarm.forwardIfPossible(decision.actionPayload)
        }
        return decision
    }
}
